...
OUT_DIR="${VCF_FAST_BENCH_OUT_DIR:-tests/output/benchmark-results/v19-second-public-format-cohort}"
REPORT="${VCF_FAST_V19_REPORT:-benchmark/reports/v19-second-public-format-cohort.md}"
FORMAT_COHORT_VCF="${VCF_FAST_FORMAT_CATTLE29_VCF:-tests/output/public-data/Dutch_Genebank_Cattle_Y_merged.vcf.gz}"
FORMAT_COHORT_URL="https://ftp.sra.ebi.ac.uk/vol1/analysis/ERZ184/ERZ18456468/Dutch_Genebank_Cattle_Y_merged.vcf.gz"
FORMAT_COHORT_ENA_ACCESSION="ERZ18456468"
FORMAT_COHORT_STUDY_ACCESSION="PRJEB60909"
FORMAT_COHORT_BYTES="131795380"
FORMAT_COHORT_RECORDS="not pre-indexed"
FORMAT_COHORT_SAMPLES="29"
FORMAT_COHORT_SPECIES="Bos taurus"
FORMAT_COHORT_LABEL="Dutch Genebank Cattle"
PUBLIC_DATA="${VCF_FAST_SECOND_FORMAT_VCF:-${VCF_FAST_FORMAT_VCF:-$FORMAT_COHORT_VCF}}"
PUBLIC_SOURCE_URL="${VCF_FAST_SECOND_FORMAT_VCF_URL:-${VCF_FAST_FORMAT_VCF_URL:-$FORMAT_COHORT_URL}}"
TIERS="${VCF_FAST_V19_TIERS:-10000 100000 full}"
RUNS="${VCF_FAST_V19_RUNS:-${VCF_FAST_BENCH_RUNS:-3}}"
WARMUP="${VCF_FAST_V19_WARMUP:-${VCF_FAST_BENCH_WARMUP:-1}}"
HEAVY_OUTPUT_RECORDS="${VCF_FAST_V19_HEAVY_OUTPUT_RECORDS:-500000}"
...
selected_sample() {
  local vcf="$1"
  if [[ -n "${VCF_FAST_V19_SAMPLE:-}" ]]; then
    echo "$VCF_FAST_V19_SAMPLE"
  else
...
ENA \`$FORMAT_COHORT_ENA_ACCESSION\` / \`$FORMAT_COHORT_STUDY_ACCESSION\`,
\`Dutch_Genebank_Cattle_Y_merged.vcf.gz\`: a 29-sample
\`$FORMAT_COHORT_SPECIES\` Y-chromosome VCF from $FORMAT_COHORT_LABEL with
declared \`FORMAT/AD\`, \`FORMAT/DP\`, and \`FORMAT/GQ\`,
\`$FORMAT_COHORT_BYTES\` bytes, and no bundled tabix index.
...
(\`VCF_FAST_V19_RUNS=$RUNS\`, \`VCF_FAST_V19_WARMUP=$WARMUP\`). Peak RSS is
...
| N_PASS(FORMAT/AD[1] > 10) >= 2 | $PUBLIC_DATA | n/a | n/a | \`bcftools filter -i 'N_PASS(FMT/AD[*:1]>10)>=2'\` | not yet proven | n/a | n/a | n/a | not yet proven | benchmark did not run |
...
if ! public_vcf_has_required_format; then
  ...
  append_not_yet_proven_rows
  echo "" >>"$REPORT"
  echo "Required FORMAT metadata was not found; use \`benchmark/download_public_data.sh format-cattle29\` or set \`VCF_FAST_SECOND_FORMAT_VCF\` to a cohort declaring FORMAT/AD, FORMAT/DP, and FORMAT/GQ." >>"$REPORT"
  exit 0
fi
...
for tier in $TIERS; do
  ...
  subset_candidate="${OUT_DIR}/second-format-cohort-${label}.vcf.gz"
  ...
  cases=(
    ...
    "N_PASS(FORMAT/AD[1] > 10) >= 2|N_PASS(FORMAT/AD[1] > 10) >= 2|N_PASS(FMT/AD[*:1]>10)>=2|"
