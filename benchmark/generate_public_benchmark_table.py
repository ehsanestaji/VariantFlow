...
ROWS = [
    ...
    Row(
        ...
        dataset="Bounded IGSR chr22 BGZF `FILTER` and AF predicates",
        correctness="default and indexed outputs matched byte-for-byte; indexed core records matched bcftools",
        measured_result='v3.0 public matrix measured `FILTER == "PASS"` at 122.95x to 272.73x faster than bcftools and `AF > 0.99` with chunk target 256 at 4.70x to 4.93x faster than bcftools through the 1M requested tier',
        source="benchmark/reports/v24-index-pushdown-benchmark.md",
        required_tokens=(
            'FILTER == "PASS"',
            "122.95x",
            "272.73x",
            ...
            "focused 100k single-run row",
        ),
        caveat="high-skip predicates only; public QUAL and coarser AF chunk targets fell back or did not beat bcftools",
    ),
    Row(
    ...
    Row(
        ...
        dataset="Bounded IGSR chr22 public-heavy BGZF and deterministic stress VCF row-group/query matrix",
        correctness="DuckDB predicate/grouped results matched normalized bcftools baselines",
        measured_result="v12 measured 3.18x to 25.67x faster amortized on bounded public-heavy rows; v26 measured 1.19x to 19.20x faster amortized and 2.51x to 240.27x query-only on deterministic stress rows",
        source="benchmark/reports/v26-columnar-pushdown-benchmark.md",
        required_tokens=("1.19x", "19.20x", "2.51x", "240.27x", "matched grouped counts"),
        caveat="native selected-column Parquet only; v26 rows are deterministic stress VCFs; Polars/PyArrow pending",
    ),
    Row(
    ...
    Row(
        ...
        dataset="1000 Genomes / IGSR 3202-sample human biallelic cohort through 1M requested tier",
        correctness="normalized VCFtools parity passed for supported diploid biallelic rows",
        measured_result="925730-record 1M tier measured 3.71x faster for frequency, 8.12x for missingness, 1.15x to 1.49x for pi/Tajima summaries, 9.88x for LD, and 1.37x for Weir-Cockerham Fst; HWE was 0.98x",
        source="benchmark/reports/v25-packed-genotype-benchmark.md",
        required_tokens=(
            "925730",
            "3.71x",
            "8.12x",
            ...
            "0.98x",
            "Weir-Cockerham Fst",
            "passed VCFtools parity and 1M tier output normalization",
            "LD R2 parity uses 1e-4 tolerance",
        ),
        caveat="scoped to supported diploid biallelic rows and official AFR/EUR population files; HWE was slightly slower at 1M and Tajima's D used higher peak RSS",
    ),
]
