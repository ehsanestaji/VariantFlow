...
ROWS = [
    ...
    Row(
        workflow="Threaded BGZF input",
        dataset="IGSR chr22 public-heavy BGZF 100k and bounded 1M",
        correctness="native modes matched byte-for-byte; core records matched bcftools",
        measured_result="2.44x to 2.55x faster than default native; 15.13x to 15.63x faster than bcftools filter",
        source="benchmark/reports/v12-public-parallel-workflow-benchmark.md",
        required_tokens=("2.44x", "2.55x", "15.13x", "15.63x"),
    ...
    Row(
        ...
        measured_result="1.91x to 1.98x faster than default native; 4.85x to 5.10x faster than bcftools filter",
        source="benchmark/reports/v12-public-parallel-workflow-benchmark.md",
        required_tokens=("1.91x", "1.98x", "4.85x", "5.10x"),
        caveat="synthetic CPU-heavy stress shape; public FORMAT-heavy rows pending",
    ),
    Row(
