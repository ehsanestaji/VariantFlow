...
ROWS = [
    ...
    Row(
        ...
        caveat="synthetic CPU-heavy stress shape; first public FORMAT-rich trio rows are mixed",
    ),
    Row(
        workflow="Public FORMAT aggregate filter",
        dataset="NA12878 trio 10k/50k N_PASS(FORMAT/AD[1] > 10)",
        correctness="matched bcftools core records",
        measured_result="50k tier 5.81x faster than bcftools filter; 10k tier was 0.68x",
        source="benchmark/reports/v17-public-format-baselines.md",
        required_tokens=("5.81x", "0.68x", "matched core records"),
        caveat="single-run macOS timing; small 10k tier is slower, larger public cohort rows pending",
    ),
    Row(
