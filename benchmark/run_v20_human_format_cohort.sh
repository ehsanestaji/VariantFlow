...
OUT_DIR="${VCF_FAST_BENCH_OUT_DIR:-tests/output/benchmark-results/v20-human-format-cohort}"
REPORT="${VCF_FAST_V20_REPORT:-benchmark/reports/v20-human-format-cohort.md}"
FORMAT_COHORT_URL="https://ddbj.nig.ac.jp/public/public-human-genomes/CHM13/JointCall/CHM13_autosome_PAR.chr22.vcf.gz"
# Index URL: https://ddbj.nig.ac.jp/public/public-human-genomes/CHM13/JointCall/CHM13_autosome_PAR.chr22.vcf.gz.csi
FORMAT_COHORT_CSI_URL="${FORMAT_COHORT_URL}.csi"
FORMAT_COHORT_BYTES="27232829080"
FORMAT_COHORT_RECORDS="remote bounded tiers only"
FORMAT_COHORT_SAMPLES="3715"
FORMAT_COHORT_SPECIES="Homo sapiens"
FORMAT_COHORT_LABEL="DDBJ CHM13 public-human-genomes JointCall chr22"
PUBLIC_DATA="${VCF_FAST_HUMAN_FORMAT_VCF:-${VCF_FAST_FORMAT_VCF:-$FORMAT_COHORT_URL}}"
PUBLIC_SOURCE_URL="${VCF_FAST_HUMAN_FORMAT_VCF_URL:-${VCF_FAST_FORMAT_VCF_URL:-$FORMAT_COHORT_URL}}"
TIERS="${VCF_FAST_V20_TIERS:-1000 10000 50000}"
RUNS="${VCF_FAST_V20_RUNS:-${VCF_FAST_BENCH_RUNS:-3}}"
WARMUP="${VCF_FAST_V20_WARMUP:-${VCF_FAST_BENCH_WARMUP:-1}}"
HEAVY_OUTPUT_RECORDS="${VCF_FAST_V20_HEAVY_OUTPUT_RECORDS:-500000}"
...
stream_public_vcf() {
  case "$PUBLIC_DATA" in
    http://*|https://*)
      curl -L --fail --silent --show-error "$PUBLIC_DATA" 2>/dev/null | gzip -cd
      ;;
    *.gz|*.bgz)
      gzip -cd "$PUBLIC_DATA"
      ;;
...
selected_sample() {
  local vcf="$1"
  if [[ -n "${VCF_FAST_V20_SAMPLE:-}" ]]; then
    echo "$VCF_FAST_V20_SAMPLE"
  else
...
prepare_tier_dataset() {
  ...
  case "$tier" in
    full|all|full-chromosome)
      if [[ "$PUBLIC_DATA" == http://* || "$PUBLIC_DATA" == https://* ]]; then
        if [[ "${VCF_FAST_ALLOW_REMOTE_FULL:-0}" != "1" ]]; then
          echo "full remote human cohort tier requires VCF_FAST_ALLOW_REMOTE_FULL=1 or a local VCF_FAST_HUMAN_FORMAT_VCF" >&2
          return 2
        fi
      fi
      echo "$PUBLIC_DATA"
...
\`$FORMAT_COHORT_SPECIES\` cohort VCF with declared \`FORMAT/AD\`,
\`FORMAT/DP\`, and \`FORMAT/GQ\`, \`$FORMAT_COHORT_BYTES\` compressed bytes,
and a CSI index at \`$FORMAT_COHORT_CSI_URL\`.
...
(\`VCF_FAST_V20_RUNS=$RUNS\`, \`VCF_FAST_V20_WARMUP=$WARMUP\`). Peak RSS is
...
if [[ "$PUBLIC_DATA" != http://* && "$PUBLIC_DATA" != https://* && ! -f "$PUBLIC_DATA" ]]; then
  write_header
  append_not_yet_proven_rows
  exit 0
fi
...
if ! public_vcf_has_required_format; then
  ...
  append_not_yet_proven_rows
  echo "" >>"$REPORT"
  echo "Required FORMAT metadata was not found; use the default DDBJ CHM13 URL or set \`VCF_FAST_HUMAN_FORMAT_VCF\` to a human cohort declaring FORMAT/AD, FORMAT/DP, and FORMAT/GQ." >>"$REPORT"
  exit 0
fi
...
for tier in $TIERS; do
  ...
  subset_candidate="${OUT_DIR}/human-format-cohort-${label}.vcf.gz"
