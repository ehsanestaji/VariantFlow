...
Heavy-output policy: tiers with at least `500000` records, and `full` / `all`
/ `full-chromosome` tiers, avoid giant plain VCF artifacts. They stream
correctness checks through `/dev/stdout` into core records only and send timed
output to `/dev/null`.
...
| public FORMAT-heavy | tests/output/public-data/19.filtered_intersect.vcf.gz | 1000000 requested / 1000000 actual | `./target/release/variantflow filter tests/output/benchmark-results/v17-public-format-heavy-full/format-public-1000000.vcf.gz --where N_PASS\(FORMAT/AD\[1\]\ \>\ 10\)\ \>=\ 2 -o /dev/stdout ` | `bcftools filter -Ov -i N_PASS\(FMT/AD\[\*:1\]\>10\)\>=2 tests/output/benchmark-results/v17-public-format-heavy-full/format-public-1000000.vcf.gz -o /dev/stdout ` | matched core records | VariantFlow 12.223630s +/- 0.282072s; bcftools 42.800560s +/- 0.870197s; speedup 3.50x | VariantFlow 4012; bcftools 7084 | measured faster on this public FORMAT-rich tier | FORMAT-rich public trio/cohort source: https://ftp.sra.ebi.ac.uk/vol1/analysis/ERZ324/ERZ324584/19.filtered_intersect.vcf.gz; samples=453; expression uses N_PASS(FORMAT/AD[1] > 10) >= 2; compare against bcftools filter; output policy: heavy output mode: /dev/stdout core records only for correctness; /dev/null for timed runs; Mayo VCF-Miner lists 1KG chr22 benchmark VCFs with 629 samples; use VCF_FAST_FORMAT_VCF after caching and validating FORMAT/AD or FORMAT/DP. |
| public FORMAT-heavy | tests/output/public-data/19.filtered_intersect.vcf.gz | full requested / 1097167 actual | `./target/release/variantflow filter tests/output/public-data/19.filtered_intersect.vcf.gz --where N_PASS\(FORMAT/AD\[1\]\ \>\ 10\)\ \>=\ 2 -o /dev/stdout ` | `bcftools filter -Ov -i N_PASS\(FMT/AD\[\*:1\]\>10\)\>=2 tests/output/public-data/19.filtered_intersect.vcf.gz -o /dev/stdout ` | matched core records | VariantFlow 14.263744s +/- 0.765175s; bcftools 45.903928s +/- 2.267056s; speedup 3.22x | VariantFlow 4196; bcftools 7100 | measured faster on this public FORMAT-rich tier | FORMAT-rich public trio/cohort source: https://ftp.sra.ebi.ac.uk/vol1/analysis/ERZ324/ERZ324584/19.filtered_intersect.vcf.gz; samples=453; expression uses N_PASS(FORMAT/AD[1] > 10) >= 2; compare against bcftools filter; output policy: heavy output mode: /dev/stdout core records only for correctness; /dev/null for timed runs; Mayo VCF-Miner lists 1KG chr22 benchmark VCFs with 629 samples; use VCF_FAST_FORMAT_VCF after caching and validating FORMAT/AD or FORMAT/DP. |
