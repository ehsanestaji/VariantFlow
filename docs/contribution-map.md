...
| Query-aware `.vfi` indexed filtering | beats bcftools on measured skip-heavy synthetic rows and bounded public IGSR AF rows; mixed vs default native | later baseline | later baseline | BGZF virtual-offset `.vfi` filtering preserves default-native byte output and matches bcftools core records; synthetic 1M skip-heavy filtering measured `6.06x` faster than default native and `55.80x` faster than bcftools, while bounded public IGSR `AF > 0.99` rows measured `3.13x` to `3.69x` faster than bcftools but slower than default native | `benchmark/reports/v21-indexed-filter-benchmark.md`, `benchmark/reports/v21-public-indexed-filter-benchmark.md` | Public AF rows skipped only `0.0%` to `50.0%` of chunks; index acceleration needs high-skip predicates or deeper BGZF scheduling to beat default native on public data. |
