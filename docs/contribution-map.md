...
| v1.4 public parallel scale policy | `src/io.rs`, `benchmark/reports/v14-public-parallel-scale-benchmark.md` | `bcftools filter` | Default native `.vcf.gz` BGZF reads now use auto-capped native BGZF workers; measured public 100k and bounded 191526-record rows matched correctness and showed auto BGZF `2.26x` to `2.39x` faster than forced single-thread native and `13.44x` to `13.47x` faster than `bcftools`; stress FORMAT aggregate rows showed opt-in predicate parallelism `1.77x` to `2.01x` faster than default native and `4.33x` to `5.27x` faster than `bcftools` | Public tier is bounded chr22:1-20000000, requested 1M reached 191526 available records; RSS was `n/a` on this macOS run; larger public FORMAT-heavy predicate evidence pending |
| v1.7 public FORMAT-rich aggregate evidence | `benchmark/reports/v17-public-format-baselines.md` | `bcftools filter` | First FORMAT-rich public trio rows matched `bcftools` core records; 50k `N_PASS(FORMAT/AD[1] > 10)` tier was `5.81x` faster, while 10k was `0.68x` | Single-run macOS timing; default dataset has three samples, not a larger cohort; larger FORMAT-rich public cohort rows pending |
...
- Public real-cohort runtime evidence for v0.9 arbitrary expression parity beyond the first FORMAT-rich trio row.
- Threaded native BGZF evidence beyond bounded IGSR chr22 1M, including whole-cohort public rows.
- Parallel ordinary gzip, parallel native output compression, and larger public FORMAT-heavy cohort evidence.
...
| Public FORMAT-heavy aggregate predicates | mixed: first FORMAT-rich public trio rows match correctness; 50k beats `bcftools`, 10k trails | later baseline | later baseline | first public FORMAT-rich evidence is correctness-positive with mixed runtime | `benchmark/reports/v17-public-format-baselines.md` | Single-run macOS timing; larger cohort and repeated Linux evidence pending. |
...
2. Expand public v0.9 expression parity beyond the first FORMAT-rich trio row before broadening the deterministic stress runtime claim for arbitrary `INFO/<KEY>`, selected `FORMAT/<KEY>`, or sample `ANY`/`ALL` predicates.
