...
| v0.9/v1.8 native expression parity | `tests/expr_tests.rs`, `tests/filter_cli_tests.rs`, `benchmark/reports/v09-expression-parity-benchmark.md`, `benchmark/reports/v18-public-format-expression-breadth.md` | `bcftools filter` | Arbitrary `INFO/<KEY>`, selected-sample `FORMAT/<KEY>`, and `ANY(FORMAT/<KEY>)` / `ALL(FORMAT/<KEY>)` semantics are documented and fixture-tested; deterministic stress rows measured `2.41x` to `5.18x` faster at 100k records, and Docker/Linux public ENA 453-sample DP/GQ/AD/selected-sample/mixed FORMAT rows measured `3.22x` to `8.77x` faster with matched core records | Public evidence is currently one FORMAT-rich cohort; htslib-backed aggregate predicates remain unsupported |
...
- Additional public real-cohort runtime evidence beyond the measured 453-sample ENA FORMAT-rich expression rows.
- Threaded native BGZF evidence beyond bounded IGSR chr22 1M, including whole-cohort public rows.
- Parallel ordinary gzip, parallel native output compression, and additional public FORMAT-heavy cohorts beyond ENA ERZ324584.
...
| Native expression parity | beats measured deterministic stress expression cases and measured public ENA 453-sample FORMAT breadth rows while matching filtered core records | later baseline | later baseline | supports arbitrary `INFO/<KEY>`, selected-sample `FORMAT/<KEY>`, and native `ANY`/`ALL` aggregate predicates; measured stress and public FORMAT expression wins | `tests/expr_tests.rs`, `tests/filter_cli_tests.rs`, `benchmark/reports/v09-expression-parity-benchmark.md`, `benchmark/reports/v18-public-format-expression-breadth.md` | htslib-backed aggregate predicates and additional public cohorts are pending. |
| Public FORMAT-heavy aggregate predicates | beats measured 453-sample public cohort tiers through full chromosome and matches correctness | later baseline | later baseline | repeated Docker/Linux ENA Ovis aries evidence is correctness-positive and faster on v1.7 AD aggregate tiers and v1.8 DP/GQ/AD/selected-sample/mixed FORMAT expression tiers | `benchmark/reports/v17-public-format-baselines.md`, `benchmark/reports/v18-public-format-expression-breadth.md` | 1M/full tiers use heavy-output mode with core-record correctness and `/dev/null` timed output; additional public cohorts are pending. |
...
2. Expand public expression parity to additional FORMAT-rich cohorts and add optional GATK/VCFtools comparisons before broadening claims beyond the ENA 453-sample public evidence.
