...
| LD packed genotype storage | matches existing VCFtools-compatible LD output in tests; performance evidence pending | complements | later baseline | LD dosage windows now use two-bit packed diploid biallelic dosage/missingness storage instead of one byte per sample | `src/engine/popgen.rs`, `tests/popgen_cli_tests.rs` | Requires a repeated VCFtools LD RSS/runtime rerun before updating speed or memory claims. |
