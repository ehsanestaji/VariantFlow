...
- v1.4 public parallel scale: Native BGZF input now uses auto-capped reader workers by default, with `VCF_FAST_NATIVE_BGZF_THREADS=1` as the single-thread escape hatch. The v1.4 harness tracks single-thread BGZF, auto BGZF, auto BGZF plus predicate parallelism, explicit BGZF threads, and `bcftools`; the tracked report is a 100-record smoke only, so it adds correctness scaffolding but no new speed claim.
...
| v1.4 public parallel scale policy | `src/io.rs`, `benchmark/reports/v14-public-parallel-scale-benchmark.md` | `bcftools filter` | Default native `.vcf.gz` BGZF reads now use auto-capped native BGZF workers; benchmark harness separates single-thread, auto BGZF, auto+predicate-parallel, explicit BGZF, and `bcftools` modes | Tracked report is 100-record smoke only; full 100k/1M v1.4 rows are pending before new speed claims |
...
- Parallel ordinary gzip, parallel native output compression, and public FORMAT-heavy predicate evidence.
...
3. Run the full v1.4 public parallel scale command at 100k/1M tiers, then update README/table claims only where correctness matches.
4. Tune auto BGZF thread caps only from full v1.4 rows; keep predicate parallelism opt-in unless public FORMAT-heavy evidence proves a safe default.
5. Run a real tagged v1.4 release once CI is green and confirm uploaded Linux/macOS archives.
