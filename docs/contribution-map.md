...
| v2.0 human FORMAT-rich cohort | `benchmark/reports/v20-human-format-cohort.md` | `bcftools filter` | DDBJ CHM13 public-human-genomes 3715-sample chr22 bounded rows for `ANY(FORMAT/DP > 20)`, `ALL(FORMAT/GQ >= 30)`, `N_PASS(FORMAT/AD[1] > 10) >= 10`, selected-sample `FORMAT/DP > 20`, and `QUAL > 30 && ANY(FORMAT/DP > 20)` matched `bcftools` core records and measured `6.45x` to `12.67x` faster | Bounded streaming tier only; full 27 GB remote VCF is not cached by default |
...
- Additional full-file public human or plant FORMAT-rich runtime evidence beyond the bounded DDBJ human row and measured ENA sheep/cattle FORMAT-rich expression rows.
...
| Public FORMAT-heavy aggregate predicates | beats measured 453-sample sheep, 29-sample cattle, and bounded 3715-sample human public cohort rows and matches correctness | later baseline | later baseline | repeated ENA Ovis aries, Dutch Genebank Cattle, and DDBJ CHM13 evidence is correctness-positive and faster on measured AD, DP, GQ, selected-sample, and mixed FORMAT expression tiers | `benchmark/reports/v17-public-format-baselines.md`, `benchmark/reports/v18-public-format-expression-breadth.md`, `benchmark/reports/v19-second-public-format-cohort.md`, `benchmark/reports/v20-human-format-cohort.md` | Heavy-output mode uses core-record correctness and `/dev/null` timed output; human evidence is bounded to 1000 records. |
...
2. Scale the DDBJ CHM13 human FORMAT-rich benchmark to larger bounded tiers, then add optional GATK/VCFtools comparisons before broadening claims beyond the sheep/cattle/human public evidence.
