...
| v0.9/v1.9 native expression parity | `tests/expr_tests.rs`, `tests/filter_cli_tests.rs`, `benchmark/reports/v09-expression-parity-benchmark.md`, `benchmark/reports/v18-public-format-expression-breadth.md`, `benchmark/reports/v19-second-public-format-cohort.md` | `bcftools filter` | Arbitrary `INFO/<KEY>`, selected-sample `FORMAT/<KEY>`, and `ANY(FORMAT/<KEY>)` / `ALL(FORMAT/<KEY>)` semantics are documented and fixture-tested; deterministic stress rows measured `2.41x` to `5.18x` faster at 100k records, Docker/Linux public ENA 453-sample DP/GQ/AD/selected-sample/mixed FORMAT rows measured `3.22x` to `8.77x`, and ENA Dutch Genebank Cattle 29-sample full-file rows measured `1.46x` to `26.66x` faster with matched core records | Public FORMAT evidence now spans two public cohorts, but the second is not human/plant-scale; htslib-backed aggregate predicates remain unsupported |
...
- Additional public human or plant FORMAT-rich runtime evidence beyond the measured ENA sheep and cattle FORMAT-rich expression rows.
- Threaded native BGZF evidence beyond bounded IGSR chr22 1M, including whole-cohort public rows.
- Parallel ordinary gzip, parallel native output compression, and additional human/plant public FORMAT-heavy cohorts beyond ENA ERZ324584 and ERZ18456468.
...
| Native expression parity | beats measured deterministic stress expression cases and measured public ENA sheep/cattle FORMAT breadth rows while matching filtered core records | later baseline | later baseline | supports arbitrary `INFO/<KEY>`, selected-sample `FORMAT/<KEY>`, and native `ANY`/`ALL` aggregate predicates; measured stress and public FORMAT expression wins | `tests/expr_tests.rs`, `tests/filter_cli_tests.rs`, `benchmark/reports/v09-expression-parity-benchmark.md`, `benchmark/reports/v18-public-format-expression-breadth.md`, `benchmark/reports/v19-second-public-format-cohort.md` | htslib-backed aggregate predicates and human/plant FORMAT-rich cohorts are pending. |
| Public FORMAT-heavy aggregate predicates | beats measured 453-sample sheep and 29-sample cattle public cohort rows and matches correctness | later baseline | later baseline | repeated ENA Ovis aries and Dutch Genebank Cattle evidence is correctness-positive and faster on measured AD, DP, GQ, selected-sample, and mixed FORMAT expression tiers | `benchmark/reports/v17-public-format-baselines.md`, `benchmark/reports/v18-public-format-expression-breadth.md`, `benchmark/reports/v19-second-public-format-cohort.md` | Heavy-output mode uses core-record correctness and `/dev/null` timed output; second cohort is not human/plant-scale. |
...
2. Find a public human or plant FORMAT-rich cohort that is directly downloadable, then add optional GATK/VCFtools comparisons before broadening claims beyond the ENA sheep/cattle public evidence.
