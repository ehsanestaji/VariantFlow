...
| v1.7 public FORMAT-rich aggregate evidence | `benchmark/reports/v17-public-format-baselines.md` | `bcftools filter` | ENA Ovis aries 453-sample FORMAT-rich cohort rows matched `bcftools` core records; Docker/Linux repeated 10k/50k/100k/250k `N_PASS(FORMAT/AD[1] > 10)` tiers measured `1.76x` to `2.79x` faster | Source file has 1,097,167 records; 1M/full-chromosome FORMAT cohort run is pending because it generates large uncompressed outputs |
...
- Public real-cohort runtime evidence for v0.9 arbitrary expression parity beyond the measured 453-sample `N_PASS(FORMAT/AD[1] > 10)` rows.
- Threaded native BGZF evidence beyond bounded IGSR chr22 1M, including whole-cohort public rows.
- Parallel ordinary gzip, parallel native output compression, and 1M/full-chromosome public FORMAT-heavy cohort evidence.
...
| Public FORMAT-heavy aggregate predicates | beats measured 453-sample public cohort tiers and matches correctness | later baseline | later baseline | repeated Docker/Linux ENA Ovis aries evidence is correctness-positive and faster on 10k/50k/100k/250k | `benchmark/reports/v17-public-format-baselines.md` | 1M/full-chromosome FORMAT cohort run is pending because outputs are large. |
