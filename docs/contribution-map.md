...
| Public parallel/workflow expansion | `benchmark/reports/v12-public-parallel-workflow-benchmark.md` | default native, parallel native, threaded BGZF input, `bcftools filter`, normalized `bcftools` query/view baselines | v1.2 smoke validates public-heavy parallel filter modes, stress parallel mode, and DuckDB `QUAL`, `INFO/DP`, `FILTER`, and grouped `CHROM,FILTER` result checks | 100-record smoke only; full 100k/1M evidence is pending before new speed claims |
...
- Parallel ordinary gzip, parallel native output compression, and public parallel predicate evidence.
- Parquet export and repeated-query performance beyond the current bounded IGSR row-count workflow, including full-tier richer DuckDB/Polars/PyArrow workflows.
...
| Public parallel plus richer columnar checks | smoke correctness matches native/`bcftools` baselines for public-heavy filter modes and DuckDB query results | complements | complements heavier cohort workflows | v1.2 harness is ready and smoke-tested; no broad speed claim from smoke rows | `benchmark/reports/v12-public-parallel-workflow-benchmark.md` | Full 100k/1M public-heavy and stress runs are pending before claim upgrades. |
...
3. Run the full v1.2 public-heavy and stress evidence command at 100k/1M tiers, then update README claims only for correctness-matched rows.
4. Expand v1.2 columnar workflow baselines beyond DuckDB to Polars/PyArrow after the full DuckDB predicate/grouped runs are measured.
