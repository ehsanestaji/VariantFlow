...
- v1.1 adds native predicate parallelism through `VCF_FAST_NATIVE_FILTER_THREADS`, evaluating bounded batches in parallel and writing passing original lines back in input order.
- v2.2 changes the default scheduler policy: unset or `auto` keeps site-only filters single-threaded and auto-enables capped ordered workers for CPU-heavy FORMAT aggregate predicates, while `1` or an explicit positive integer keeps reproducible manual control.
- v2.2 adds a scheduler benchmark harness that compares forced single-thread, default auto, BGZF-only, predicate-only, combined BGZF+predicate scheduling, and `bcftools` with RSS and CPU-second fields before updating claims.
- v2.3 starts the native scheduler rewrite by replacing stop-and-go batch evaluation with a bounded ordered pipeline, allowing reading/BGZF decode to overlap predicate evaluation while preserving output order.
...
- v2.5 VCFtools population evidence: On the accepted 925730-record 1000 Genomes / IGSR public diploid biallelic tier with official AFR/EUR population files, VariantFlow matched normalized VCFtools outputs for frequency, missingness, HWE, heterozygosity, site pi, window pi, Tajima's D, LD, and Weir-Cockerham Fst. SUPERSEDED by v2.9 (`benchmark/reports/v29-m5max-popgen-reproduction.md`), measured on the Apple M5 Max named in the manuscript: frequency `3.68x`, missingness `8.49x`/`8.04x`, LD `5.86x`, heterozygosity `1.46x`, Hardy-Weinberg `1.23x` faster, site pi `1.08x`, Tajima's D `1.27x`, Fst `1.09x`.
...
- Broader VCFtools replacement evidence beyond supported diploid biallelic normalized rows, additional population-file combinations, exact HWE p-value output, and multiallelic edge behavior.
...
| VCFtools-style population summaries | complements | beats most measured supported 1000 Genomes / IGSR public rows through the accepted 925730-record tier | later baseline | supported diploid biallelic `freq`, `missingness`, `hardy`, `het`, `pi`, `tajima-d`, `ld`, and Weir-Cockerham `fst` match normalized VCFtools rows; the 1M requested tier measured wins for frequency, missingness, heterozygosity, pi/window-pi/Tajima summaries, LD, and Fst | `tests/popgen_cli_tests.rs`, `make vcftools-parity`, `benchmark/reports/vcftools-popgen-parity-benchmark.md`, `benchmark/reports/v25-packed-genotype-benchmark.md` | Scoped to supported diploid biallelic rows and official AFR/EUR population files; HWE exact p-value is outside current output, and HWE was slightly slower at 1M. |
...
| Query-aware `.vfi` indexed filtering | beats bcftools on measured skip-heavy synthetic rows and guarded bounded public IGSR AF rows; raw index use is reserved for high-skip plans | later baseline | later baseline | BGZF virtual-offset `.vfi` filtering preserves default-native byte output and matches bcftools core records; planner metadata now covers CHROM/POS ranges, exact FILTER values, arbitrary numeric INFO bounds, FORMAT-key absence, and previous QUAL/DP/AF fields | `benchmark/reports/v21-indexed-filter-benchmark.md`, `benchmark/reports/v21-public-indexed-filter-benchmark.md` | Public AF rows skipped only `0.0%` to `50.0%` of chunks; new pushdown fields need measured v2.5 rows before claim expansion. |
| Parallel native predicate evaluation | beats measured default-native and `bcftools` CPU-heavy stress filter rows while preserving record order | later baseline | later baseline | auto-scheduled ordered-batch parallel evaluation now targets CPU-heavy aggregate FORMAT filters, with explicit `VCF_FAST_NATIVE_FILTER_THREADS=1/<N>/auto` control, bounded ordered pipeline execution, and a v2.2 scheduler report scaffold for full public/stress rows | `src/engine/filter.rs`, `benchmark/reports/v11-parallel-native-filter-benchmark.md`, `benchmark/reports/v22-scheduler-benchmark.md` | Synthetic stress evidence supports the optimization; full v2.2/v2.3 public BGZF scheduler rows are pending before broader defaults expand. |
| LD packed genotype storage | matches VCFtools-compatible LD output and has accepted 1M public evidence | beats measured bounded-distance LD row | later baseline | LD dosage windows use two-bit packed diploid biallelic dosage/missingness storage; the 925730-record IGSR tier measured `5.86x` faster than VCFtools on the Apple M5 Max (v2.9); the earlier `9.88x` was a Docker/Linux result and is superseded | `src/engine/popgen.rs`, `tests/popgen_cli_tests.rs`, `benchmark/reports/v25-packed-genotype-benchmark.md` | Scoped to bounded `--max-distance 500` diploid biallelic LD. The v2.9 M5 Max run compared 15,232,310 pairs byte-identically; the `1e-4` tolerance applies only to the earlier v1.7 normalized-dosage parity check. |
...
2. Use the accepted v2.5 1M VCFtools evidence to guide the next popgen work: HWE speed, exact HWE p-value output, multiallelic edge policy, and additional population-file combinations.
3. Run the full v2.2 scheduler report and tune auto BGZF/predicate caps from repeated public rows; keep site-only filters single-threaded by default unless public evidence proves a safe broader default.
