...
- Public real-cohort runtime evidence for v0.9 arbitrary expression breadth beyond the measured 453-sample `N_PASS(FORMAT/AD[1] > 10)` rows.
- Threaded native BGZF evidence beyond bounded IGSR chr22 1M, including whole-cohort public rows.
- Parallel ordinary gzip, parallel native output compression, and public FORMAT-heavy cohort rows for more aggregate expressions.
...
| Public FORMAT-heavy aggregate predicates | beats measured 453-sample public cohort tiers through full chromosome and matches correctness | later baseline | later baseline | repeated Docker/Linux ENA Ovis aries evidence is correctness-positive and faster on 10k/50k/100k/250k/1M/full-chromosome | `benchmark/reports/v17-public-format-baselines.md` | 1M/full tiers use heavy-output mode with core-record correctness and `/dev/null` timed output; broader public aggregate expressions are pending. |
...
2. Expand public v0.9 expression parity beyond the measured ENA 453-sample `N_PASS(FORMAT/AD[1] > 10)` rows before broadening the deterministic stress runtime claim for arbitrary `INFO/<KEY>`, selected `FORMAT/<KEY>`, or sample `ANY`/`ALL` predicates.
