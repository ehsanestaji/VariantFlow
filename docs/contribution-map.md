...
- v2.5 VCFtools population evidence: On the accepted 925730-record 1000 Genomes / IGSR public diploid biallelic tier with official AFR/EUR population files, VariantFlow matched normalized VCFtools outputs for frequency, missingness, HWE, heterozygosity, site pi, window pi, Tajima's D, LD, and Weir-Cockerham Fst. It measured `3.71x` faster for frequency, `8.12x` for missingness, `1.15x` to `1.49x` for pi/Tajima summaries, `9.88x` for LD, and `1.37x` for Weir-Cockerham Fst; HWE was correctness-matched but slightly slower at `0.98x`.
...
| LD packed genotype storage | matches VCFtools-compatible LD output and has accepted 1M public evidence | beats measured bounded-distance LD row | later baseline | LD dosage windows use two-bit packed diploid biallelic dosage/missingness storage; the 925730-record IGSR tier measured `9.88x` faster than VCFtools with comparable peak RSS | `src/engine/popgen.rs`, `tests/popgen_cli_tests.rs`, `benchmark/reports/v25-packed-genotype-benchmark.md` | Scoped to bounded `--max-distance 500` diploid biallelic LD; LD R2 parity uses `1e-4` tolerance for near-zero floating differences. |
