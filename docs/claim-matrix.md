...
| Public FORMAT-heavy aggregate filtering | beats on measured 453-sample cohort tiers | `benchmark/reports/v17-public-format-baselines.md` reports matched core records on an ENA Ovis aries 453-sample FORMAT-rich cohort; 10k/50k/100k/250k were 1.76x to 2.79x faster than `bcftools filter` | bcftools | Docker/Linux timing; 1M/full-chromosome FORMAT cohort run is pending because outputs are large |
