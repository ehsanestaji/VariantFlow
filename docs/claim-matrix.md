...
| Native FORMAT aggregate stress filtering | beats | `benchmark/reports/v14-public-parallel-scale-benchmark.md` reports byte-for-byte native parity and 4.33x to 5.27x over `bcftools filter` | bcftools | deterministic stress data; public FORMAT-rich WGS trio evidence is positive but only three samples |
| Public FORMAT-heavy aggregate filtering | beats on measured WGS trio tiers | `benchmark/reports/v17-public-format-baselines.md` reports matched core records on FORMAT-rich Zenodo WGS trio tiers; 10k/50k/100k/250k were 3.01x to 5.89x faster than `bcftools filter` | bcftools | repeated macOS timing; three samples only; larger FORMAT-rich cohort evidence pending |
