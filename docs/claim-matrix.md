...
| Native FORMAT aggregate stress filtering | beats | `benchmark/reports/v14-public-parallel-scale-benchmark.md` reports byte-for-byte native parity and 4.33x to 5.27x over `bcftools filter` | bcftools | deterministic stress data; first public FORMAT-rich trio evidence is mixed |
| Public FORMAT-heavy aggregate filtering | mixed | `benchmark/reports/v17-public-format-baselines.md` reports matched core records on FORMAT-rich NA12878 trio tiers; 50k was 5.81x faster than `bcftools filter`, while 10k was 0.68x | bcftools | first public FORMAT-rich row; single-run macOS timing; larger cohort evidence pending |
