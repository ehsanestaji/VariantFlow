...
correctness-matched benchmarks, VariantFlow computed missingness on the
1000 Genomes 3\,202-sample high-coverage dataset 8.0--8.5x faster than VCFtools
across chromosomes~22 and~1 at a constant 8.6~MB of memory, evaluated an
index-assisted $\mathtt{FILTER}=\mathtt{PASS}$ predicate 123--273x faster than
bcftools where block metadata excludes most of the file, and computed
bounded-distance linkage disequilibrium 5.9x faster; it filtered FORMAT-rich predicates 4.74--17.78x
faster than bcftools on subsets of 1\,000 to 50\,000 records streamed from a
3\,715-sample chromosome~22 call set; and it served
export-once DuckDB queries 1.19--19.20x faster on a repeated-query workload. The
core population-genetic outputs were byte-identical to VCFtools, per-individual
missingness and the site-frequency spectrum matched scikit-allel exactly, and
the missing-data-aware $\pi$ and $d_{XY}$ estimators reproduced pixy's pairwise
counts in every window. By decoding only the fields
each operation requires, VariantFlow brings large-cohort post-calling analysis
within reach of interactive, exploratory work on a single workstation. It
...
(file dated 2025-12-19; CSI index alongside). All datasets are used under their
...
\midrule
Human FORMAT filters & CHM13 chr22, 3\,715 sam., 1k--50k rec. & 4.74--17.78x & 8.1--9.0\,/\,11.6--15.1 MB & \checkmark \\
Indexed \texttt{PASS} & IGSR chr22, 100k--1M & 123--273x & 4.5--6.1\,/\,4.2--4.4 MB & \checkmark \\
Indexed AF pred. & IGSR chr22, 100k--1M, 256-rec & 4.70--4.93x & 9.9--66.1\,/\,4.4 MB & \checkmark \\
Missingness & IGSR chr22 full, 1.07M & 8.49x & 8.6\,/\,4.1 MB & \checkmark \\
Missingness & IGSR chr1 full, 6.47M & 8.04x & 8.7\,/\,4.1 MB & \checkmark \\
LD ($r^{2}$) & IGSR chr22, 925k & 5.86x & 9.0\,/\,4.2 MB & \checkmark \\
