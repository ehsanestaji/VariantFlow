...
against a reference tool (Supplementary Table~S1); operations that trail a
competitor are treated as optimisation targets rather than claims. The benchmark
system configuration is given in Supplementary Table~S2, and the per-workflow
timings and memory for the streaming population-genetics and repeated-query
workflows in Supplementary Table~S3.

On a 3\,715-sample chromosome~22 call set aligned to the T2T-CHM13 reference
(DDBJ), filters over FORMAT-rich predicates produced the same records as bcftools
while running 4.74--17.78x faster. These filtering measurements were taken on
...
allele-frequency threshold predicate ran 4.70--4.93x faster. The
...
8.49x faster than VCFtools on chromosome~22 (1,073,369 records) and 8.04x faster
on chromosome~1 (6,468,094 records), at a peak resident memory of 8.6~MB that
was independent of chromosome size. Bounded-distance linkage disequilibrium
($r^{2}$, \texttt{--max-distance 500}) ran 5.9x faster on a 925,730-record
dataset. The remaining VCFtools-compatible summaries were faster by smaller
