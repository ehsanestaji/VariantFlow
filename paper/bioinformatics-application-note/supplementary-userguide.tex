...
\subsection{Site-frequency spectrum (\texttt{sfs})}
\paragraph{Verification note.} The \texttt{sfs} subcommand is present in the
VariantFlow~1.5.0 release binary. Its folded and unfolded spectra were validated
against scikit-allel~1.3.13 and match bin for bin (maximum absolute difference
zero); see \texttt{benchmark/reports/v30-correctness-crosschecks.md}. An earlier
note stating that \texttt{sfs} was absent from the prebuilt binary referred to a
pre-1.5.0 build and no longer applies.
