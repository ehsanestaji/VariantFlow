...
| v1.4 stress FORMAT aggregate scale | default native / `bcftools filter` | parallel output matched default native byte-for-byte and matched `bcftools` filtered core records | opt-in predicate parallelism was `1.77x` to `2.01x` faster than default native and `4.33x` to `5.27x` faster than `bcftools` on `ANY(FORMAT/AD > 80)` through 1M stress records | deterministic stress data; public 453-sample FORMAT cohort evidence is now tracked separately in v1.7/v1.8; RSS was `n/a` on this macOS run |
| v2.4 query-aware `.vfi` indexed filtering | default native / `bcftools filter` | indexed output matched default native byte-for-byte and matched `bcftools` core records | synthetic skip-heavy 1M remained a strong indexed win; public IGSR `FILTER == "PASS"` measured `122.95x` to `272.73x` faster than `bcftools`; public IGSR `AF > 0.99` with chunk target 256 measured `4.70x` to `4.93x` faster than `bcftools` through the 1M requested tier | high-skip public predicates only; public QUAL and coarser AF chunk targets fell back or did not beat `bcftools` |
| v2.5 VCFtools-style public population summaries | `VCFtools` | normalized parity passed for supported diploid biallelic rows | 925730-record 1M IGSR tier measured `3.71x` faster for frequency, `8.12x` for missingness, `1.15x` to `1.49x` for pi/Tajima summaries, `9.88x` for LD, and `1.37x` for Weir-Cockerham Fst; HWE was `0.98x` | official AFR/EUR population files; HWE slightly slower; Tajima's D used higher peak RSS; LD R2 parity uses `1e-4` tolerance |
| v1.7 public FORMAT aggregate evidence | `bcftools filter` | matched filtered core records on ENA Ovis aries 453-sample FORMAT-rich cohort tiers | Docker/Linux repeated 10k/50k/100k/250k/1M/full-chromosome `N_PASS(FORMAT/AD[1] > 10)` tiers were `1.76x` to `3.50x` faster than `bcftools` | 1M/full tiers use heavy-output mode: correctness streams core records only and timed output goes to `/dev/null` |
| v1.8 public FORMAT expression breadth | `bcftools filter` | matched filtered core records on ENA Ovis aries 453-sample FORMAT-rich cohort 1M/full tiers | Docker/Linux repeated `ANY(FORMAT/DP > 20)`, `ALL(FORMAT/GQ >= 30)`, `N_PASS(FORMAT/AD[1] > 10) >= 10`, selected-sample `FORMAT/DP > 20`, and mixed `QUAL > 30 && ANY(FORMAT/DP > 20)` rows were `3.22x` to `8.77x` faster than `bcftools` | hyperfine reported outliers on some rows; heavy-output mode avoids retained full VCF artifacts |
| v1.9 second public FORMAT-rich cohort | `bcftools filter` | matched filtered core records on ENA Dutch Genebank Cattle 29-sample full Y-chromosome VCF | repeated full-file `ANY(FORMAT/DP > 20)`, `ALL(FORMAT/GQ >= 30)`, `N_PASS(FORMAT/AD[1] > 10) >= 2`, selected-sample `FORMAT/DP > 20`, and mixed `QUAL > 30 && ANY(FORMAT/DP > 20)` rows were `1.46x` to `26.66x` faster than `bcftools` | non-sheep second cohort, but not human/plant; Mayo human 629-sample VCF-Miner downloads returned 403 during automated validation |
| v2.0 human FORMAT-rich cohort | `bcftools filter` | matched filtered core records on DDBJ CHM13 3715-sample human chr22 VCF bounded tiers | repeated 1k/10k/50k `ANY(FORMAT/DP > 20)`, `ALL(FORMAT/GQ >= 30)`, `N_PASS(FORMAT/AD[1] > 10) >= 10`, selected-sample `FORMAT/DP > 20`, and mixed `QUAL > 30 && ANY(FORMAT/DP > 20)` rows were `4.74x` to `17.78x` faster than `bcftools` | bounded streaming tiers only; harness does not cache the 27 GB remote VCF by default |
...
- `benchmark/reports/v21-indexed-filter-benchmark.md`
- `benchmark/reports/v21-public-indexed-filter-benchmark.md`
- `benchmark/reports/v22-scheduler-benchmark.md`
- `benchmark/reports/v25-packed-genotype-benchmark.md`
- `benchmark/reports/vcftools-popgen-parity-benchmark.md`
- `docs/contribution-map.md`

Public evidence now supports the native selective-filter claim on measured GIAB and IGSR tiers. The v0.7 heavy run also shows the optimized native TSV path can beat `bcftools query` on bounded sample-rich gzip workloads through 1M records, v0.9 stress evidence shows the expanded native expression engine beating `bcftools filter` on measured deterministic stress cases, and the v1.0 slices show opt-in threaded native BGZF input, typed Parquet export, and a measured bounded-IGSR row-count workflow win through DuckDB. The first v1.1 slice adds native predicate evaluation for CPU-heavy expressions while preserving byte-for-byte output, v2.2 adds a full scheduler benchmark harness for forced single-thread, default auto, BGZF-only, predicate-only, combined, and `bcftools` rows, and v2.3 starts the native scheduler rewrite by overlapping reading/BGZF decode with bounded ordered predicate evaluation. v1.2 moves parallel evidence into public-heavy BGZF filter rows, 1M stress parallel rows, and richer DuckDB predicate/grouped workflow checks. v1.4 completes the next public scale pass: default auto BGZF input wins the bounded public I/O-heavy QUAL filter, while predicate parallelism wins the CPU-heavy FORMAT aggregate stress case. v1.7 through v2.0 now add repeated public FORMAT-rich cohort rows: correctness matches `bcftools` on the ENA 453-sample Ovis aries chromosome 19 VCF, a second ENA Dutch Genebank Cattle 29-sample full Y-chromosome VCF, and DDBJ CHM13 3715-sample human chr22 bounded 1k/10k/50k tiers, with aggregate AD and broader DP/GQ/selected-sample/mixed FORMAT expressions winning on the measured rows. The VCFtools population-genetics layer now matches VCFtools on supported diploid biallelic fixture rows and on true public 1000 Genomes / IGSR AFR/EUR population rows through the accepted 925730-record 1M tier: frequency, missingness, pi/Tajima summaries, LD, and Weir-Cockerham Fst are faster on measured rows, while HWE is correctness-matched but slightly slower at 1M. Honest gaps remain: BCF TSV still trails `bcftools query`, ordinary gzip is not parallelized, the human FORMAT evidence is bounded rather than a full 27 GB run, GATK external baselines are later rows, VCFtools evidence is still scoped to diploid biallelic normalized rows, and Polars/PyArrow plus larger whole-cohort columnar evidence are still pending.
...
v2.5 1000 Genomes / IGSR true-population evidence extends this to official
AFR/EUR population files through an accepted 925730-record 1M tier. The current
public rows are faster than VCFtools on most measured supported cases, but HWE
is slightly slower at 1M and this is still not a broad VCFtools replacement
claim.
...
`index` writes an explicit `.vfi` JSON sidecar with chunk-level metadata for query-aware skipping: record counts, genomic ranges, exact FILTER values, QUAL min/max, INFO/DP and INFO/AF bounds, arbitrary numeric INFO bounds when present, FORMAT key presence, and BGZF virtual offsets when the input is BGZF. Ordinary commands still work without the sidecar. Current evidence is intentionally scoped: skip-heavy synthetic BGZF filtering is a measured indexed speed win, and a focused bounded IGSR `AF > 0.99` row shows finer chunking can skip most public chunks; low-skip public predicates still fall back safely and do not become speed claims.
