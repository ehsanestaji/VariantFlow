...
| v2.1 query-aware `.vfi` indexed filtering | default native / `bcftools filter` | indexed output matched default native byte-for-byte and matched `bcftools` core records | synthetic skip-heavy 1M was `6.06x` faster than default native and `55.80x` faster than `bcftools`; bounded public IGSR `AF > 0.99` rows now use a guarded planner that falls back on low-skip cases and measured `10.01x` to `13.31x` faster than `bcftools` | public IGSR rows skipped only `0.0%` to `50.0%` of chunks, so raw index acceleration still needs high-skip predicates |
...
`index` writes an explicit `.vfi` JSON sidecar with chunk-level metadata for query-aware skipping: record counts, genomic ranges, exact FILTER values, QUAL min/max, INFO/DP and INFO/AF bounds, arbitrary numeric INFO bounds when present, FORMAT key presence, and BGZF virtual offsets when the input is BGZF. Ordinary commands still work without the sidecar. Current evidence is intentionally scoped: skip-heavy synthetic BGZF filtering is a measured indexed speed win, while bounded public IGSR `AF > 0.99` rows prove the guarded planner can decline low-skip `.vfi` plans and fall back to default native filtering without changing output.
