...
| v2.0 human FORMAT-rich cohort | `bcftools filter` | matched filtered core records on DDBJ CHM13 3715-sample human chr22 VCF bounded tier | repeated 1000-record `ANY(FORMAT/DP > 20)`, `ALL(FORMAT/GQ >= 30)`, `N_PASS(FORMAT/AD[1] > 10) >= 10`, selected-sample `FORMAT/DP > 20`, and mixed `QUAL > 30 && ANY(FORMAT/DP > 20)` rows were `6.45x` to `12.67x` faster than `bcftools` | bounded streaming tier only; harness does not cache the 27 GB remote VCF by default |
