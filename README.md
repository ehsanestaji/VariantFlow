...
`index` writes an explicit `.vfi` JSON sidecar with chunk-level metadata for query-aware skipping: record counts, genomic ranges, FILTER values, QUAL min/max, INFO/DP min/max, INFO/AF presence, FORMAT key presence, and BGZF virtual offsets when the input is BGZF. Ordinary commands still work without the sidecar. Current evidence is intentionally scoped: skip-heavy synthetic BGZF filtering is a measured indexed speed win, while bounded public IGSR `AF > 0.99` rows prove the guarded planner can decline low-skip `.vfi` plans and fall back to default native filtering without changing output.
