...
| v1.4 stress FORMAT aggregate scale | default native / `bcftools filter` | parallel output matched default native byte-for-byte and matched `bcftools` filtered core records | opt-in predicate parallelism was `1.77x` to `2.01x` faster than default native and `4.33x` to `5.27x` faster than `bcftools` on `ANY(FORMAT/AD > 80)` through 1M stress records | deterministic stress data; public FORMAT-rich WGS trio evidence is positive but only three samples; RSS was `n/a` on this macOS run |
| v1.7 public FORMAT aggregate evidence | `bcftools filter` | matched filtered core records on FORMAT-rich Zenodo WGS trio tiers | repeated 10k/50k/100k/250k `N_PASS(FORMAT/AD[1] > 10)` tiers were `3.01x` to `5.89x` faster than `bcftools` | repeated macOS timing with RSS; still three samples, so larger FORMAT-rich cohort evidence is pending |
