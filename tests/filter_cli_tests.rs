...
#[test]
fn indexed_filter_falls_back_when_skip_estimate_is_too_low() {
    let dir = tempdir().unwrap();
    let input = dir.path().join("stress.vcf.gz");
    let index = PathBuf::from(format!("{}.vfi", input.display()));
    let output = dir.path().join("filtered.vcf");
    ...
    let report: Value = serde_json::from_str(&fs::read_to_string(report).unwrap()).unwrap();
    assert_eq!(report["indexed"], Value::Bool(false));
    assert!(
    ...
    assert_eq!(report["chunks_skipped"].as_u64().unwrap(), 0);
    assert_eq!(report["records_skipped_estimate"].as_u64().unwrap(), 0);
}
...
#[test]
fn empty_source_matching_index_falls_back_to_default_streaming_output() {
    let dir = tempdir().unwrap();
    let input = dir.path().join("stress.vcf.gz");
    let index = PathBuf::from(format!("{}.vfi", input.display()));
    let default_output = dir.path().join("default.vcf");
    ...
    let mut json: Value = serde_json::from_str(&fs::read_to_string(&index).unwrap()).unwrap();
    json["record_count"] = Value::from(0);
    json["chunks"] = Value::Array(Vec::new());
    fs::write(&index, serde_json::to_string_pretty(&json).unwrap()).unwrap();
...
#[test]
fn shifted_index_start_falls_back_to_default_streaming_output() {
    let dir = tempdir().unwrap();
    let input = dir.path().join("stress.vcf.gz");
    let index = PathBuf::from(format!("{}.vfi", input.display()));
    let default_output = dir.path().join("default.vcf");
    ...
    let first_chunk = &mut json["chunks"][0];
    let virtual_start = first_chunk["virtual_start"].as_u64().unwrap();
    first_chunk["virtual_start"] = Value::from(virtual_start + 10);
    fs::write(&index, serde_json::to_string_pretty(&json).unwrap()).unwrap();
